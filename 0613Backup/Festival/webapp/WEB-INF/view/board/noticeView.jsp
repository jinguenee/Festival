...
<body>
		...
		<section>
			<div class="sub_wrap">
					...
					<div class="sub_content">
						<div class="write_table">
							<table>
								...
								<tr>
									<th>첨부파일</th>
									<td colspan="3"><c:forEach var="bean" items="${fileList}" varStatus="status">
									<c:if test="${fileList ne null}"> <a target="_blank" href="${bean.fileImgPath}">${bean.fileOriginName}</a></c:if>
									</c:forEach></td>
								</tr>
								<tr>
									<th >내용</th>
									<td colspan="3" id="boardContent"><c:forEach var="bean" items="${fileList}" varStatus="status">
										<c:if test="${fileList ne null}"><img width="100%" src="${bean.fileImgPath}" ></c:if>
										</c:forEach>
						...
						<tr>
							<td><span class="form-inline" role="form">
									<div class="form-group">
										<label>[댓글]</label> <br/>
										<label>[유저명]&nbsp;&nbsp;:&nbsp;&nbsp;<input type="text" disabled="disabled" value="${sessionScope.memberBean.memberName}" /> 
										&nbsp; <button type="button" id="commentOk" class="btn btn-default">완료</button> </label>
									</div>
				...
				<div class="container_reply">
					<table>
						...
						<label>[댓글 리스트]</label> <br/>
						<c:forEach var="bean" items="${boardReplyList}" varStatus="status"> 
						<c:choose>
							<c:when test="${bean.memberNo eq sessionScope.memberBean.memberNo or !empty sessionScope.memberBean.memberAdmin}">
							<a style="cursor:pointer;" onclick="deleteReply(${bean.brNo});">${bean.memberName} : ${bean.brContent}</a>
							<br/></c:when> 
							<c:otherwise>
            ...
            <!--  로그인이 되어 있을 때  -->
            <c:if test="${boardList.memberNo eq sessionScope.memberBean.memberNo or !empty sessionScope.memberBean.memberAdmin}">
           ...
           <!--  로그인이 안 되어 있을 때 -->
           <c:if test="${boardList.memberNo ne sessionScope.memberBean.memberNo and empty sessionScope.memberBean.memberAdmin}">
              <button class="btn_back" onclick="location.href='noticeList.do'">뒤로가기</button>
           </c:if>
                </div>
            </div>
