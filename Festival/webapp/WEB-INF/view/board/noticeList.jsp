...
<head>
	...
	$(function(){
        $.ajax({
           type: "post",
           url: "/Festival/board/noticeListProc.do",
           data:{
              pageNo: "${param.pageNo}"
           },
           dataType: "json",
           success: function(data) {
              ...
              if(data.result == "ok") {
                 ...
                 $.each(data.boardBean, function(i, mBean) {
                    ...
                    str += "<td><a href='/Festival/board/noticeView.do?boardNo="+mBean.boardNo+"&'>" + mBean.boardTitle + "</a></td>";
                    str += "<td>" + "</td>";
			...
			<div class="sub_wrap">
				...
				<div class="sub_nav">
					...
					<div class="sub_content">
						<div class="btn_wrap">
							<button class="btn_write" onclick="location.href='/Festival/board/noticeWrite.do' " >글쓰기</button>
							<div class="find">
								<input type="text" class="input" />
								<button type="button" class="btn_find">검색하기</button>
							</div>
						</div>
						...
						<div class="page">
							<ul>
							 <c:if test="${pagingBean.groupNo > 1}">
					               <li><a href="/Festival/board/noticeList.do?pageNo=${pagingBean.pageStartNo-1}" class="on">&lt;</a></li>
				            </c:if>
				            <c:forEach var="i" begin="${pagingBean.pageStartNo}" end="${pagingBean.pageEndNo}">
				            <c:choose>
				               <c:when test="${pagingBean.pageNo != i}">
				                  <li><a href="/Festival/board/noticeList.do?pageNo=${i}" class="on">${i}</a></li>
				               </c:when>
				               <c:otherwise>
				                  <li class="on">${i}</li>
				               </c:otherwise>
				            ...
				            <c:if test="${pagingBean.groupNo < pagingBean.totalGroupCount}">
				               <li><a href="/Festival/board/noticeList.do?pageNo=${pagingBean.pageEndNo+1}" class="on">&gt;</a></li>
				            </c:if>
